export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'search' || !action) {
      ...
      // Appel 1: Google Local → hôtel de référence + GPS
      ...
      // Appel 2: Google Local zone → concurrents avec adresses
      ...
      // Appel 3: Google Hotels par nom exact → token fiable pour la référence
      const refSearch = await serpFetch({
        engine: 'google_hotels', q: ref.title || q,
        check_in_date: ci, check_out_date: co,
        adults: '2', currency: 'EUR', hl: 'fr', gl: 'fr', api_key
      });
      const refProps = refSearch.properties || [];
      const refBest = refProps[0] || {};

      const refHotel = {
        name: ref.title || q,
        overall_rating: ref.rating || refBest.overall_rating || null,
        hotel_class: refBest.hotel_class || null,
        address: ref.address || null,
        gps_coordinates: ref.gps_coordinates || null,
        property_token: refBest.property_token || null,
        isReference: true
      };

      // Pour chaque concurrent: chercher par nom exact dans Google Hotels
      const exclude = /camping|auberge|hostel|gite|résidence|appartement|studio/i;
      const seen = new Set([(ref.title || q).toLowerCase()]);
      const hotelZone = zoneResults.filter(h => {
      ...
      // Recherche par nom exact pour chaque concurrent (par lots de 4)
      const competitors = [];
      ...
      for (const chunk of chunks) {
        const results = await Promise.all(chunk.map(async h => {
          try {
            const r = await serpFetch({
              engine: 'google_hotels', q: h.title,
              check_in_date: ci, check_out_date: co,
              adults: '2', currency: 'EUR', hl: 'fr', gl: 'fr', api_key
            });
            const props = r.properties || [];
            // Prendre le premier résultat qui n'est pas la référence
            const best = props.find(p => p.property_token !== refBest.property_token) || props[0] || {};
            return {
              name: h.title,
              overall_rating: h.rating || best.overall_rating || null,
              hotel_class: best.hotel_class || null,
              address: h.address || null,
              gps_coordinates: h.gps_coordinates || null,
              property_token: best.property_token || null,
              isReference: false
            };
          } catch(e) {
            return {
        ...
        // Filtrer les doublons de token
        results.forEach(r => {
          if (r.property_token && competitors.some(c => c.property_token === r.property_token)) {
            r.property_token = null; // token déjà utilisé = confusion, on supprime
          }
          competitors.push(r);
        });
      }
    ...
    if (action === 'details') {
      ...
      const data = await serpFetch({
        engine: 'google_hotels', q: q || 'hotel',
        property_token, check_in_date, check_out_date,
        adults: '2', currency: 'EUR', hl: 'fr', gl: 'fr', api_key
      });
      ...
      if (!prices.length && prop.rate_per_night?.extracted_lowest)
    ...
    if (action === 'timeseries') {
      ...
      const results = await Promise.all(offsets.map(async offset => {
        try {
          const ci = getTomorrow(offset);
          const co = getTomorrow(offset + 1);
          const data = await serpFetch({
            engine: 'google_hotels', q: q || 'hotel',
            property_token, check_in_date: ci, check_out_date: co,
            adults: '2', currency: 'EUR', hl: 'fr', gl: 'fr', api_key
          });
          const prop = data.properties?.[0] || {};
          ...
          return { date: ci, offset: `J+${offset}`, price: lowest };
        } catch(e) {
          return { date: getTomorrow(offset), offset: `J+${offset}`, price: null };
        }
      }));
