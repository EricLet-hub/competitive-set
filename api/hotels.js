export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'search' || !action) {
      ...
      // Appel 1: Google Local — liste les hôtels de la zone avec adresses réelles
      ...
      // Fonction: trouver le token Google Hotels via adresse ou nom exact
      // Utilise une recherche dédiée par adresse pour éviter toute confusion
      async function getTokenForHotel(name, address) {
        const searchQ = address ? `${name} ${address}` : name;
        const r = await serpFetch({
          engine: 'google_hotels', q: searchQ,
          check_in_date: ci, check_out_date: co,
          adults: '2', currency: 'EUR', hl: 'fr', gl: 'fr', api_key
        });
        const props = r.properties || [];
        if (!props.length) return { token: null, hotel_class: null, rating: null };
        // Prendre le premier résultat (la recherche par adresse est très précise)
        const best = props[0];
        return {
          token: best.property_token || null,
          hotel_class: best.hotel_class || null,
          rating: best.overall_rating || null,
          name: best.name || name
        };
      }
      ...
      // Référence
      const refData = await getTokenForHotel(ref.title || q, ref.address);
      const refHotel = {
        name: ref.title || q,
      ...
      // Concurrents — chercher token par adresse pour chaque hôtel
      const seen = new Set([normName(ref.title || q)]);
      const competitors = [];
      const refToken = refData.token;

      // Limiter à 8 hôtels pour ne pas dépasser les crédits SerpAPI
      ...
      // Récupérer les tokens en parallèle (max 4 en même temps pour éviter rate limit)
      const chunks = chunkArray(hotelZone, 4);
      for (const chunk of chunks) {
        const results = await Promise.all(chunk.map(h => getTokenForHotel(h.title, h.address)));
        chunk.forEach((h, i) => {
          const d = results[i];
          // Exclure si même token que la référence
          if (d.token && d.token === refToken) return;
          competitors.push({
            name: h.title,
